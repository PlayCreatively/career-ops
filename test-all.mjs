...
try {
  ...
  // GameJobs.co keeps expired postings in its sitemap for a while, flagged
  // "This job might no longer be available" — the "might" must not slip past the
  // "this job (is )?no longer" pattern. The liveness pass is what drops them.
  const staleGameJobs = classifyLiveness({
    finalUrl: 'https://gamejobs.co/Unity-Developer-at-Virtuos-9730',
    bodyText: 'This job might no longer be available. Unity Developer Virtuos Ukraine, Kyiv 15 days ago Apply Save We invite an experienced Unity Developer to join our team...',
    applyControls: ['Apply'],
  });
  if (staleGameJobs.result === 'expired' && staleGameJobs.code === 'expired_body') {
    pass('GameJobs.co "This job might no longer be available" flash is caught (not revived by its Apply button)');
  } else {
    fail(`GameJobs.co stale posting misclassified as ${staleGameJobs.result}/${staleGameJobs.code}`);
  }
} catch (e) {
